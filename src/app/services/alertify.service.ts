...
export class AlertifyService {
  ...
  prepareMapData(dataArr: any[], currentTab: string) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (dataArr[0].CountryofOrigin) {
        countryKey = "CountryofOrigin";
      } else if (dataArr[0].CountryofDestination) {
        countryKey = "CountryofDestination";
      } else {
        console.warn("No country field found in data");
        return resolve([]);
      }

      console.log("prepareMapData", dataArr, currentTab);
